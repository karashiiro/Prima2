    ...
    public class FFXIV3RoleQueueService
    {
        private static string QueuePath => Environment.OSVersion.Platform == PlatformID.Win32NT
            ...
            : Path.Combine(Environment.GetEnvironmentVariable("HOME")
                           ?? throw new ArgumentException("No HOME variable set!"), "queues.json");
        ...
        {
            await AlertTimeouts(GetOrCreateQueue("learning-and-frag-farm").Timeout(QueueInfo.BAQueueTimeout, 0 * Time.Second, excludeEvents: true), "learning-and-frag-farm", 4);
            await AlertTimeouts(GetOrCreateQueue("av-and-ozma-prog").Timeout(QueueInfo.BAQueueTimeout, 0 * Time.Second, excludeEvents: true), "av-and-ozma-prog", 4);
            await AlertTimeouts(GetOrCreateQueue("clears-and-farming").Timeout(QueueInfo.BAQueueTimeout, 0 * Time.Second, excludeEvents: true), "clears-and-farming", 4);
            await AlertTimeouts(GetOrCreateQueue("lfg-castrum").Timeout(QueueInfo.CastrumQueueTimeout, 0 * Time.Second, excludeEvents: true), "lfg-castrum", 4);
            await AlertTimeouts(GetOrCreateQueue("lfg-delubrum-savage").Timeout(QueueInfo.DelubrumQueueTimeout, 0 * Time.Second, excludeEvents: true), "lfg-delubrum-savage", 4);
            await AlertTimeouts(GetOrCreateQueue("lfg-drs-fresh-prog").Timeout(QueueInfo.DelubrumQueueTimeout, 0 * Time.Second, excludeEvents: true), "lfg-drs-fresh-prog", 4);
            await AlertTimeouts(GetOrCreateQueue("lfg-delubrum-normal").Timeout(QueueInfo.DelubrumQueueTimeout, 0 * Time.Second, excludeEvents: true), "lfg-delubrum-normal", 4);
            await AlertTimeouts(GetOrCreateQueue("lfg-dalriada").Timeout(QueueInfo.DalriadaQueueTimeout, 0 * Time.Second, excludeEvents: true), "lfg-dalriada", 4);
        }
