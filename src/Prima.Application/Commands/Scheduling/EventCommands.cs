...
public class EventCommands : ModuleBase<SocketCommandContext>
{
    ...
    public EventCommands(IDbService db, MuteService mutes)
    {
        _db = db;
        _mutes = mutes;
    }
    ...
    public async Task PinMessage(string messageRef = "")
    {
        if (!Context.Channel.Name.Contains("group-chat") &&
            !new ulong[] { 766444330880598076, 858440602588020736 }.Contains(Context.Channel.Id))
        {
        ...
        await _db.AddEphemeralPin(Context.Guild.Id, Context.Channel.Id, messageId, RunHostData.PinnerRoleId,
            Context.User.Id, DateTime.UtcNow);
    ...
    public async Task UnpinMessage(string messageRef = "")
    {
        ...
        await _db.RemoveEphemeralPin(messageId);
    ...
    public async Task SetPrioritySpeaker(IUser other)
    {
        ...
        await _db.AddTimedRole(PrioritySpeakerRoleId, Context.Guild.Id, member.Id, DateTime.UtcNow.AddHours(4.5));
    ...
    public async Task MuteUser(IUser other)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Log.Error(e, "Unmute failed; deferring unmute until next connection");
                _mutes.DeferUnmute(otherMember);
            }
        });
